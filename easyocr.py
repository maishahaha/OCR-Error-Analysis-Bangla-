...
from ocr_models import ocr_easyocr

# Configuration
IMG_DIR = 'dataset/images'
LABEL_CSV = 'dataset/labels.csv'
OUTPUT_CSV = 'easyocr_results.csv'
SUMMARY_TXT = 'easyocr_summary.txt'

# Load labels
df = pd.read_csv(LABEL_CSV)
results = []

print(f"🚀 Running EasyOCR Baseline on {len(df)} images (CPU Mode)...")

for _, row in tqdm(df.iterrows(), total=len(df)):
    img_path = os.path.join(IMG_DIR, row['image_name'])
    gt = row['ground_truth_text'] # Using your confirmed column name
    
    # Run OCR
    pred, conf = ocr_easyocr(img_path)
    
    # Calculate CER with normalized logic
...
summary_content = f"""EASYOCR PERFORMANCE SUMMARY
========================================
